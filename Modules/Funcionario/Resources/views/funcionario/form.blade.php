        ...
        <div id="dependentes">
            @foreach(old('dependentes', $data['dependentes']) as $key => $dependente)
                ...
                <div class="dep {{ old('dependentes.$key.nome', isset($dependente->nome) ? $dependente->nome : '') ? '' : 'd-none'}} mb-4">
                    ...
                    <input type="checkbox" value="{{old('dependentes.$key.matricula_dependente', isset($dependente->matricula_dependente) ? $dependente->matricula_dependente : '')}}" name="dependentes[{{$key}}][matricula_dependente]" >Comprovante de matricula
                    <input type="checkbox" value="{{old('dependentes.$key.certificado_vacinacao', isset($dependente->certificado_vacinacao) ? $dependente->certificado_vacinacao : '')}}" name="dependentes[{{$key}}][certificado_vacinacao]">Certificado de vacinação
                </div>
